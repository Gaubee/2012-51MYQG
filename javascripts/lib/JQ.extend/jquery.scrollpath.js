...
(function ($, window, document, undefined) {
    ...
    function Path(scrollS, rotateS) {
        ...
        this.moveTo = function (x, y, options) {
            var settings = $.extend({}, defaults, options), //!+ W(1)
        ...
        this.arc = function (centerX, centerY, radius, startAngle, endAngle, counterclockwise, options) {
            ...
            for (; i <= steps; i++) {
                path.push({ x: centerX + radius * Math.cos(startAngle + radStep * i), //!+W(1)
    ...
    function makeCSS(node) {
        ...
        //        document.getElementById("RaphaelCanvas").style = 'margin-top:' + (-centeredY) + 'px; margin-left:' + (-centeredX) + 'px';
        //        document.getElementById("RaphaelCanvas").style = "";
        ...
        if (normalizeAngle(node.rotate) === 0) {
            ...
            alert(-centeredY + ',' + (-centeredY - H(1))+','+node.rotate);
            applyPrefix(style, "transform", "translate(" + (-centeredX + W(1) * node.rotate) + "px, " + (-centeredY - H(1) * node.rotate) + "px) rotate(" + node.rotate + "rad)"); /// <reference path="jquery.scrollpath.js" />
 //           applyPrefix(style, "transform", "translate(" + ( -W(4)) + "px, " + ( -H(1.6) ) + "px) rotate(" + node.rotate + "rad)"); /// <reference path="jquery.scrollpath.js" />
        }
